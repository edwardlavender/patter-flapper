...
states_init.StateXYD <- 
  '
  function Patter.states_init(state_type::Type{StateXYD}, coords, behaviour)
    N = nrow(coords)
    # Simulate initial coordinates
    coords.angle = rand(N) .* 2 .* π
    # Set behaviour
    # (based on change in depth over following time step)
    coords.behaviour .= behaviour[1]
    return coords
  end 
  '
...
simulate_step.ModelMoveFlapperCRW <- 
  ...
  function Patter.simulate_step(state::StateXYD, model_move::ModelMoveFlapperCRW, t::Int64, behaviour::Vector{Int64} = behaviour)
    ...
    # Simulate step from state (at t - 1), based on behaviour (at time t - 1)
    # (Behaviour at time t - 1 is based on vertical activity between t - 1 and t)
    if state.behaviour == 1
      length  = rand(model_move.dbn_length_rest)
    elseif state.behaviour == 2
      length  = rand(model_move.dbn_length_active)
    end 
    # Simulate angle 
    ...
    y         = state.y + length * sin(angle)
    map_value = Patter.extract(model_move.map, x, y)
    # Define state at time t
    # * state at time t includes behaviour at time t
    # * In logpdf_normalisation_constants(), caching is based on input state (t - 1)
    # * For each input, we simulate new states to test if they are valid (in water) 
    # * For caching, t = 1
    # * Simulated state behaviours are incorrect
    # * But this does not matter since we only check if states are valid
    StateXYD(map_value, x, y, angle, behaviour[t])
  end 
'
...
logpdf_step.ModelMoveFlapperCRW <- 
  ...
  function Patter.logpdf_step(state_from::StateXYD, state_to::StateXYD, model_move::ModelMoveFlapperCRW, 
    ...
    if state_from.behaviour == 1
      length_logpdf = logpdf(model_move.dbn_length_rest, length)
    elseif state_from.behaviour == 2
      length_logpdf = logpdf(model_move.dbn_length_active, length)
    end 
...
set_model_move_components <- function() {
  # State methods for StateXYD (for CRW)
